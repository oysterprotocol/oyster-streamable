...
var DownloadStream = (function(_Readable) {
  ...
  function DownloadStream(genesisHash, metadata, options) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (DownloadStream.__proto__ || Object.getPrototypeOf(DownloadStream)).call(
        this,
        opts
      )
    );
    ...
    if (_config.INCLUDE_TREASURE_OFFSETS) {
      _this.numChunks += Math.ceil(
        _this.numChunks / (_config.FILE.CHUNKS_PER_SECTOR - 1)
      );
    }
  ...
  _createClass(DownloadStream, [
    ...
    },
    {
      ...
      value: function _download() {
        ...
        (0, _backend.queryGeneratedSignatures)(
          iotaProvider,
          hash,
          limit,
          binaryMode
          ...
          .then(function(result) {
            ...
            // Check if finished
            if (
              _this2.numChunks - _this2.chunkOffset <= 0 &&
              _this2.ongoingDownloads === 0
            ) {
            ...
            _this2.emit(
              "progress",
              100 * (_this2.downloadedChunks / _this2.numChunks)
            );
            _this2._pushChunk();
          })
