...
class DownloadStream extends _readableStream.Readable {
  constructor(genesisHash, metadata, options) {
    ...
    if (_config.INCLUDE_TREASURE_OFFSETS) {
      this.numChunks += Math.ceil(this.numChunks / (_config.FILE.CHUNKS_PER_SECTOR - 1));
    }
  ...
  _download() {
    ...
    const limit = Math.min(this.numChunks - this.chunkOffset, this.options.chunksPerBatch);
